...
void FormatCustom(const std::vector<aur::Package> &packages, const std::string &format) {
  for (const auto &p : packages) { format::Custom(format, p); }
}

void SortUnique(std::vector<aur::Package> &packages, const sort::Sorter &sorter) {
  absl::c_sort(packages, sorter);
  packages.resize(std::unique(packages.begin(), packages.end()) - packages.begin());
}

std::vector<std::string> NotFoundPackages(const std::vector<std::string> &want, const std::vector<aur::Package> &got,
                                          const auracle::PackageCache &package_cache) {
  std::vector<std::string> missing;

  for (const auto &name : want) {
    if (package_cache.LookupByPkgname(name) != nullptr) { continue; }

    if (absl::c_find_if(got, [&](const aur::Package &pkg) { return pkg.name == name; }) != got.end()) { continue; }
...
bool ChdirIfNeeded(const fs::path &target) {
  ...
  if (ec.value() != 0) {
    std::cerr << "error: failed to change directory to " << target << ": " << ec.message() << "\n";
    return false;
  }
...
bool RpcResponseIsFailure(const aur::ResponseWrapper<aur::RpcResponse> &response) {
  const std::string error = response.ok() ? response.value().error : response.status().ToString();
...
Auracle::Auracle(Options options)
    : aur_(aur::NewAur(aur::Aur::Options().set_baseurl(options.aur_baseurl).set_useragent("Auracle/" YMERGE_VERSION))),
      pacman_(options.pacman) {}

void Auracle::IteratePackages(std::vector<std::string> args, Auracle::PackageIterator *state) {
  ...
  aur_->QueueRpcRequest(info_request,
                        [this, state, want{std::move(args)}](aur::ResponseWrapper<aur::RpcResponse> response) {
                          if (RpcResponseIsFailure(response)) { return -EIO; }

                          auto results = response.value().results;

                          for (const auto &p : NotFoundPackages(want, results, state->package_cache)) {
                            if (!pacman_->HasPackage(p)) { std::cerr << "no results found for " << p << "\n"; }
                          }

                          for (auto &result : results) {
                            // check for the pkgbase existing in our repo
                            const bool have_pkgbase = state->package_cache.LookupByPkgbase(result.pkgbase) != nullptr;
                            ...
                            if (!added || have_pkgbase) { continue; }

                            if (state->callback) { state->callback(*p); }

                            if (state->recurse) {
                              std::vector<std::string> alldeps;
                              alldeps.reserve(p->depends.size() + p->makedepends.size() + p->checkdepends.size());

                              for (auto kind : state->resolve_depends) {
                                for (const auto &dep : GetDependenciesByKind(p, kind)) { alldeps.push_back(dep.name); }
                              }
...
int Auracle::Info(const std::vector<std::string> &args, const CommandOptions &options) {
  if (args.empty()) { return ErrorNotEnoughArgs(); }

  std::vector<aur::Package> packages;
  aur_->QueueRpcRequest(aur::InfoRequest(args), [&](aur::ResponseWrapper<aur::RpcResponse> response) {
    if (RpcResponseIsFailure(response)) { return -EIO; }
  ...
  if (packages.empty()) { return -ENOENT; }
...
int Auracle::Search(const std::vector<std::string> &args, const CommandOptions &options) {
  if (args.empty()) { return ErrorNotEnoughArgs(); }
  ...
  const auto matches = [&](const aur::Package &p) {
    return absl::c_all_of(patterns, [&](const std::regex &re) {
      switch (options.search_by) {
        case aur::SearchRequest::SearchBy::NAME: return std::regex_search(p.name, re);
        case aur::SearchRequest::SearchBy::NAME_DESC:
          return std::regex_search(p.name, re) || std::regex_search(p.description, re);
        default:
  ...
  for (const auto &arg : args) {
    ...
    if (options.allow_regex) {
      ...
      if (frag.empty() && options.allow_regex) {
        std::cerr << "error: search string '" << arg << "' insufficient for searching by regular expression.\n";
        return -EINVAL;
      }
    }
    ...
    aur_->QueueRpcRequest(aur::SearchRequest(options.search_by, frag),
                          [&](aur::ResponseWrapper<aur::RpcResponse> response) {
                            ...
                            std::copy_if(std::make_move_iterator(results.begin()),
                                         std::make_move_iterator(results.end()), std::back_inserter(packages), matches);
                            return 0;
                          });
  }
...
int Auracle::Clone(const std::vector<std::string> &args, const CommandOptions &options) {
  if (args.empty()) { return ErrorNotEnoughArgs(); }

  if (!ChdirIfNeeded(options.directory)) { return -EINVAL; }

  int ret = 0;
  PackageIterator iter(options.recurse, options.resolve_depends, [this, &ret](const aur::Package &p) {
    aur_->QueueCloneRequest(
        aur::CloneRequest(p.pkgbase), [&ret, pkgbase{p.pkgbase}](aur::ResponseWrapper<aur::CloneResponse> response) {
          if (response.ok()) {
            std::cout << response.value().operation << " complete: " << (fs::current_path() / pkgbase).string() << "\n";
          } else {
            std::cerr << "error: clone failed for " << pkgbase << ": " << response.status() << "\n";
            ret = -EIO;
          }
          return 0;
        });
  ...
  if (iter.package_cache.empty()) { return -ENOENT; }

  return ret;
}

int Auracle::Show(const std::vector<std::string> &args, const CommandOptions &options) {
  if (args.empty()) { return ErrorNotEnoughArgs(); }
  ...
  aur_->QueueRpcRequest(aur::InfoRequest(args), [&](aur::ResponseWrapper<aur::RpcResponse> response) {
    ...
    for (const auto &pkg : response.value().results) {
      aur_->QueueRawRequest(aur::RawRequest::ForSourceFile(pkg, options.show_file),
                            [&options, print_header = resultcount > 1,
                             pkgbase = pkg.pkgbase](aur::ResponseWrapper<aur::RawResponse> response) {
                              if (absl::IsNotFound(response.status())) {
                                std::cerr << "error: file '" << options.show_file << "' not found for package '"
                                          << pkgbase << "'\n";
                                return -ENOENT;
                              } else if (!response.ok()) {
                              ...
                              if (print_header) {
                                std::cout << "### BEGIN " << pkgbase << "/" << options.show_file << "\n";
                              }
  ...
  if (resultcount == 0) { return -ENOENT; }
...
int Auracle::BuildOrder(const std::vector<std::string> &args, const CommandOptions &options) {
  ...
  std::vector<std::tuple<std::string, const aur::Package *, std::vector<std::string>>> total_ordering;
  absl::flat_hash_set<std::string> seen;
  for (const auto &arg : args) {
    iter.package_cache.WalkDependencies(
        arg,
        [&](const std::string &pkgname, const aur::Package *package, const std::vector<std::string> &dependency_path) {
          if (seen.emplace(pkgname).second) { total_ordering.emplace_back(pkgname, package, dependency_path); }
        },
...
int Auracle::Update(const std::vector<std::string> &args, const CommandOptions &options) {
  if (!ChdirIfNeeded(options.directory)) { return -EINVAL; }
  ...
  int ret = 0;
  PackageIterator iter(options.recurse, options.resolve_depends, [&](const aur::Package &p) {
    aur_->QueueCloneRequest(
        aur::CloneRequest(p.pkgbase), [&ret, pkgbase = p.pkgbase](aur::ResponseWrapper<aur::CloneResponse> response) {
          if (response.ok()) {
            std::cout << response.value().operation << " complete: " << (fs::current_path() / pkgbase).string() << "\n";
  ...
  for (const auto &p : packages) { outdated.push_back(p.name); }
...
int Auracle::GetOutdatedPackages(const std::vector<std::string> &args, std::vector<aur::Package> *packages) {
  aur::InfoRequest info_request;
  ...
  aur_->QueueRpcRequest(info_request, [&](aur::ResponseWrapper<aur::RpcResponse> response) {
    ...
    auto results = response.value().results;
    std::copy_if(std::make_move_iterator(results.begin()), std::make_move_iterator(results.end()),
...
int Auracle::Outdated(const std::vector<std::string> &args, const CommandOptions &options) {
  std::vector<aur::Package> packages;
  ...
  std::sort(packages.begin(), packages.end(), sort::MakePackageSorter("name", sort::OrderBy::ORDER_ASC));
...
int Auracle::RawSearch(const std::vector<std::string> &args, const CommandOptions &options) {
  for (const auto &arg : args) { aur_->QueueRawRequest(aur::SearchRequest(options.search_by, arg), &RawSearchDone); }
