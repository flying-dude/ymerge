...
template <typename... T> FMT_INLINE void println(fmt::format_string<T...> fmt, T &&...args) {
	const auto &vargs = fmt::make_format_args(args...);
	fmt::detail::is_utf8() ? vprint(fmt, vargs) : fmt::detail::vprint_mojibake(stdout, fmt, vargs);
	std::putc('\n', stdout);
}

template <typename... T> FMT_INLINE void info(fmt::format_string<T...> fmt, T &&...args) {
...
template <typename... T> FMT_INLINE void warn(fmt::format_string<T...> fmt, T &&...args) {
	if (flag::quiet) return;
...
template <typename... T> FMT_INLINE void error(fmt::format_string<T...> fmt, T &&...args) {
	fmt::print(stderr, "[");
	if (flag::color)
...
template <typename... T> FMT_INLINE void todo(fmt::format_string<T...> fmt, T &&...args) {
	fmt::print("[");
...
template <typename... T> FMT_INLINE xresult<void> exec_opt(cmd_options opt, T &&...args) {
	if (flag::quiet) {
...
template <typename... T> FMT_INLINE xresult<void> exec(T &&...args) { return exec_opt({}, args...); }

// git command shorthandle. use "-C" flag to change working dir.
template <typename... Argv> xresult<void> git(Argv &&...argv) { return exec("git", argv...); }
