...
void main_throws(int argc, const char **argv) {
  ...
  if (!exists(config::curated_aur_repo.get_path() / "git")) {
    fmt::print("Package dir \"{}\" not present.\n", (config::curated_aur_repo.get_path() / "git" / "pkg").c_str());
  ...
  exec_opt_throw("could not verify git commit.",
    {}, "sudo", "git", "-C", (config::curated_aur_repo.get_path() / "git").c_str(), "verify-commit", "HEAD");

  std::string whitelist_bytes = file_contents(config::curated_aur_repo.get_path() / "git" / "aur-whitelist.json");
